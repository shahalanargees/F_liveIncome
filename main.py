...
for duration in range(300):
    ...
    df_copy["new_age"] = df_copy["age"] * np.random.randint(1, 5, size=len(df_copy))
    df_copy["whpw_new"] = df_copy["hours-per-week"] * np.random.randint(1, 5, size=len(df_copy))

    # KPI Calculations
    avg_age = np.mean(df_copy["new_age"])

    count_married = int(
        df_copy[df_copy["marital-status"] == "Married-civ-spouse"]["marital-status"].count()
        + np.random.randint(1, 30)
    )

    hpw = np.mean(df_copy["whpw_new"])
    ...
    with place_holder.container():
        ...
        with figCol2:
            ...
            fig2 = px.histogram(df_copy, x="new_age")
